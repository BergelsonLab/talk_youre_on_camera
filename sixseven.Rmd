...
wa1 <- wilcox.test(sixseven_aud$six_numtokens, sixseven_aud$sev_numtokens, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "wa1")
wa2 <- wilcox.test(sixseven_aud$six_numtypes, sixseven_aud$sev_numtypes, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "wa2")
wa3 <- wilcox.test(sixseven_aud$six_numspeakers, sixseven_aud$sev_numspeakers, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "wa3")
wa4 <- wilcox.test(sixseven_aud$six_MOT, sixseven_aud$sev_MOT, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "wa4")
wa5 <- wilcox.test(sixseven_aud$six_FAT, sixseven_aud$sev_FAT, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "wa5")
wa6 <- wilcox.test(sixseven_aud$six_d, sixseven_aud$sev_d, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "wa6")
wa7 <- wilcox.test(sixseven_aud$six_q, sixseven_aud$sev_q, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "wa7")
wa8 <- wilcox.test(sixseven_aud$six_r, sixseven_aud$sev_r, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "wa8")
wa9 <- wilcox.test(sixseven_aud$six_s, sixseven_aud$sev_s, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "wa9")
wa10 <- wilcox.test(sixseven_aud$six_i, sixseven_aud$sev_i, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "wa10")
wa11 <- wilcox.test(sixseven_aud$six_n, sixseven_aud$sev_n, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "wa11")
wa12 <- wilcox.test(sixseven_aud$six_y_op, sixseven_aud$sev_y_op, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "wa12")
...
#none of video 6vs7 aren't different
wv1 <- wilcox.test(sixseven_vid$six_numtokens,sixseven_vid$sev_numtokens, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "wv1")
wv2 <- wilcox.test(sixseven_vid$six_numtypes,sixseven_vid$sev_numtypes, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "wv2")
wv3 <- wilcox.test(sixseven_vid$six_numspeakers,sixseven_vid$sev_numspeakers, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "wv3")
wv4 <- wilcox.test(sixseven_vid$six_MOT,sixseven_vid$sev_MOT, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "wv4")
wv5 <- wilcox.test(sixseven_vid$six_d,sixseven_vid$sev_d, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "wv5")
wv6 <- wilcox.test(sixseven_vid$six_r,sixseven_vid$sev_r, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "wv6")
wv7 <- wilcox.test(sixseven_vid$six_i,sixseven_vid$sev_i, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "wv7")
wv8 <- wilcox.test(sixseven_vid$six_y_op,sixseven_vid$sev_y_op, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "wv8")
wv9 <- wilcox.test(sixseven_vid$six_q,sixseven_vid$sev_q, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "wv9")
wv10 <- wilcox.test(sixseven_vid$six_n,sixseven_vid$sev_n, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "wv10")
wv11 <- wilcox.test(sixseven_vid$six_s,sixseven_vid$sev_s, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "wv11")
wv12 <- wilcox.test(sixseven_vid$six_FAT,sixseven_vid$sev_FAT, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "wv12")

wvs <- bind_rows(wv1, wv2, wv3, wv4, wv5, wv6, wv7, wv8, wv9, wv10 ,wv11, wv12) %>% 
  mutate(pval_adj = p.adjust(p.value, method = "holm"))
wvs %>% filter(pval_adj>.05)
# do six vs. seven audio count vars correlate: yes, all correlate
ca1 <- cor.test(sixseven_aud$six_numtokens, sixseven_aud$sev_numtokens, conf.int=T, method = "kendall") %>% tidy() %>% mutate(comp = "ca1")
ca2 <- cor.test(sixseven_aud$six_numtypes, sixseven_aud$sev_numtypes, conf.int=T, method = "kendall") %>% tidy() %>% mutate(comp = "ca2")
ca3 <- cor.test(sixseven_aud$six_numspeakers, sixseven_aud$sev_numspeakers, conf.int=T, method = "kendall") %>% tidy() %>% mutate(comp = "ca3")
ca4 <- cor.test(sixseven_aud$six_MOT, sixseven_aud$sev_MOT, conf.int=T, method = "kendall") %>% tidy() %>% mutate(comp = "ca4")
ca5 <- cor.test(sixseven_aud$six_FAT, sixseven_aud$sev_FAT, conf.int=T, method = "kendall") %>% tidy() %>% mutate(comp = "ca5")
ca6 <- cor.test(sixseven_aud$six_d, sixseven_aud$sev_d, conf.int=T, method = "kendall") %>% tidy() %>% mutate(comp = "ca6")
ca7 <- cor.test(sixseven_aud$six_q, sixseven_aud$sev_q, conf.int=T, method = "kendall") %>% tidy() %>% mutate(comp = "ca7")
ca8 <- cor.test(sixseven_aud$six_r, sixseven_aud$sev_r, conf.int=T, method = "kendall") %>% tidy() %>% mutate(comp = "ca8")
ca9 <- cor.test(sixseven_aud$six_s, sixseven_aud$sev_s, conf.int=T, method = "kendall") %>% tidy() %>% mutate(comp = "ca9")
ca10 <- cor.test(sixseven_aud$six_i, sixseven_aud$sev_i, conf.int=T, method = "kendall") %>% tidy() %>% mutate(comp = "ca10")
ca11 <- cor.test(sixseven_aud$six_n, sixseven_aud$sev_n, conf.int=T, method = "kendall") %>% tidy() %>% mutate(comp = "ca11")
ca12 <- cor.test(sixseven_aud$six_y_op, sixseven_aud$sev_y_op, conf.int=T, method = "kendall") %>% tidy() %>% mutate(comp = "ca12")

cas <- bind_rows(ca1, ca2, ca3, ca4, ca5, ca6, ca7, ca8, ca9, ca10 ,ca11, ca12) %>% 
  mutate(pval_adj = p.adjust(p.value, method = "holm"))
cas %>% filter(pval_adj>.05)

# do six vs. seven video count vars correlate: all but cv7: num_r

cv1 <- cor.test(sixseven_vid$six_numtokens, sixseven_vid$sev_numtokens, conf.int=T, method = "kendall") %>% tidy() %>% mutate(comp = "cv1")
cv2 <- cor.test(sixseven_vid$six_numtypes, sixseven_vid$sev_numtypes, conf.int=T, method = "kendall") %>% tidy() %>% mutate(comp = "cv2")
cv3 <- cor.test(sixseven_vid$six_numspeakers, sixseven_vid$sev_numspeakers, conf.int=T, method = "kendall") %>% tidy() %>% mutate(comp = "cv3")
cv4 <- cor.test(sixseven_vid$six_MOT, sixseven_vid$sev_MOT, conf.int=T, method = "kendall") %>% tidy() %>% mutate(comp = "cv4")
cv5 <- cor.test(sixseven_vid$six_FAT, sixseven_vid$sev_FAT, conf.int=T, method = "kendall") %>% tidy() %>% mutate(comp = "cv5")
cv6 <- cor.test(sixseven_vid$six_d, sixseven_vid$sev_d, conf.int=T, method = "kendall") %>% tidy() %>% mutate(comp = "cv6")
cv7 <- cor.test(sixseven_vid$six_q, sixseven_vid$sev_q, conf.int=T, method = "kendall") %>% tidy() %>% mutate(comp = "cv7")
cv8 <- cor.test(sixseven_vid$six_r, sixseven_vid$sev_r, conf.int=T, method = "kendall") %>% tidy() %>% mutate(comp = "cv8")
cv9 <- cor.test(sixseven_vid$six_s, sixseven_vid$sev_s, conf.int=T, method = "kendall") %>% tidy() %>% mutate(comp = "cv9")
cv10 <- cor.test(sixseven_vid$six_i, sixseven_vid$sev_i, conf.int=T, method = "kendall") %>% tidy() %>% mutate(comp = "cv10")
cv11 <- cor.test(sixseven_vid$six_n, sixseven_vid$sev_n, conf.int=T, method = "kendall") %>% tidy() %>% mutate(comp = "cv11")
cv12 <- cor.test(sixseven_vid$six_y_op, sixseven_vid$sev_y_op, conf.int=T, method = "kendall") %>% tidy() %>% mutate(comp = "cv12")

cvs <- bind_rows(cv1, cv2, cv3, cv4, cv5, cv6, cv7, cv8, cv9, cv10 ,cv11, cv12) %>% 
  mutate(pval_adj = p.adjust(p.value, method = "holm"))
cvs %>% filter(pval_adj>.05)
#need to rewrite all wilcox/cor.test code similar to the the way i did with the counts in the preceding chunk, or even better with forloops
```

##Derived Counts, Month 6 vs. 7
We first analyzed the 12 derived count measures in month six versus seven, by recording-type. Across children, all 12 derived count audio measures correlated significantly month-to-month (Kendall’s tau ranged from `r ifelse(sum(cas$pval_adj>0),round(min(cas$estimate),2),"FIXTHIS")` - `r ifelse(sum(cas$pval_adj>0),round(max(cas$estimate),2),"FIXTHIS")`, `r ifelse(sum(cas$pval_adj>0.05)==0, "all adjusted-p<.05","FIXTHIS")`, and did not significantly differ by month (`r ifelse(sum(was$pval_adj<0.05)==0, "all adjusted-p>.05","FIXTHIS")`). All video measures too correlated significantly month-to-month, except number of reading tokens (`r ifelse(filter(cvs, comp=="cv8")$pval_adj>.05,"adjusted-p>.05","FIXTHIS")`; remaining 11 Kendall’s tau ranged from `r round(min(subset(cvs, pval_adj<.05)$estimate),2)` - `r round(max(subset(cvs, pval_adj<.05)$estimate),2)`,  all adjusted-p<.05) For video too no derived count measures varied significantly from six to seven months `r ifelse(sum(wvs$pval_adj<0.05)==0, "all adjusted-p>.05","FIXTHIS")``. Thus, within recording type, the count-based metrics of the object words infants heard were statistically equivalent in month six and seven, and correlated significantly within children month-to-month (except 1 of the 24 correlations). This suggests that parents are likely behaving relatively naturally during recordings, or are adjusting their behavior as a function of our observations to an extremely consistent degree. 

##Derived Counts, Audio vs. Video-recordings
We next assessed how quantity of input scaled between our hour-long video recordings and daylong audio recordings. While video recordings were all around an hour (*M*=`r paste(mean(vidtime$length), ",",sd(vidtime$length), "R=",max(vidtime$length), "-", min(vidtime$length)`), the length of audio recordings varied *M*=`r paste(mean(audtime$length), ",",sd(audtime$length), "R=",max(audtime$length), "-", min(audtime$length)`(M=14.4(XX sd), R=10.5-16 hrs). The modal recording length was`r mode(audtime$length)`16 hours, the maximum capacity of the LENA devices. However, children varied in their naptime schedule, and indeed from audio alone it can be hard to know with certainty whether the child was asleep. As described in the Methods section, we demarcated all long stretches of silence and thus have an upper-limit on their wakeful (i.e. non-silent) hours (*M*=`r paste(mean(audtime$length), ",",sd(audtime$length), "R=",max(audtime$length), "-", min(audtime$length)`). This is in line with established norms for 6–8-month-olds in the US (Mandel et al, 2010), which report the average amount of sleep is 3 hours during the day, and 10 hour during the night. Thus, given a 24-hour day, 16 hours of recording beginning when the child wakes up should capture roughly 11 daytime hours, in line with our silence demarcations. 
...
Another way to compute the difference between the daylong and hourlong recordings is to simply divide the audio count by the video count. Doing this results in a range of 2.5-4  across our metrics, rather than the roughly tenfold increase one would expect if video recordings captured an average hour of the day.
NB: Still need to do stats for this section after normalizing by length

```{r 6vs 7 prop measures wilcox, echo = F, warning = F, message = F}
# none of the audio props 6 vs 7 are different
pwa1 <- wilcox.test(sixseven_aud$six_prop_op,sixseven_aud$sev_prop_op, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "pwa1")
pwa2 <- wilcox.test(sixseven_aud$six_prop_mom,sixseven_aud$sev_prop_mom, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "pwa2")
pwa3 <- wilcox.test(sixseven_aud$six_prop_dad,sixseven_aud$sev_prop_dad, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "pwa3")
pwa4 <- wilcox.test(sixseven_aud$six_propd,sixseven_aud$sev_propd, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "pwa4")
pwa5 <- wilcox.test(sixseven_aud$six_propi,sixseven_aud$sev_propi, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "pwa5")
pwa6 <- wilcox.test(sixseven_aud$six_propr,sixseven_aud$sev_propr, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "pwa6")
pwa7 <- wilcox.test(sixseven_aud$six_propq,sixseven_aud$sev_propq, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "pwa7")
pwa8 <- wilcox.test(sixseven_aud$six_props,sixseven_aud$sev_props, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "pwa8")
pwa9 <- wilcox.test(sixseven_aud$six_propn,sixseven_aud$sev_propn, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "pwa9")
pwa10 <- wilcox.test(sixseven_aud$six_type_token_ratio,sixseven_aud$sev_type_token_ratio, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "pwa10")
...
pwv1 <- wilcox.test(sixseven_vid$six_prop_op,sixseven_vid$sev_prop_op, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "pwv1")
pwv2 <- wilcox.test(sixseven_vid$six_prop_mom,sixseven_vid$sev_prop_mom, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "pwv1")
pwv3 <- wilcox.test(sixseven_vid$six_prop_dad,sixseven_vid$sev_prop_dad, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "pwv1")
pwv4 <- wilcox.test(sixseven_vid$six_propd,sixseven_vid$sev_propd, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "pwv1")
pwv5 <- wilcox.test(sixseven_vid$six_propi,sixseven_vid$sev_propi, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "pwv1")
pwv6 <- wilcox.test(sixseven_vid$six_propr,sixseven_vid$sev_propr, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "pwv1")
pwv7 <- wilcox.test(sixseven_vid$six_propq,sixseven_vid$sev_propq, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "pwv1")
pwv8 <- wilcox.test(sixseven_vid$six_props,sixseven_vid$sev_props, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "pwv1")
pwv9 <- wilcox.test(sixseven_vid$six_propn,sixseven_vid$sev_propn, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "pwv1")
pwv10 <- wilcox.test(sixseven_vid$six_type_token_ratio,sixseven_vid$sev_type_token_ratio, conf.int=T, paired = T) %>% tidy() %>% mutate(comp = "pwv1")
...
```{r cor tests 6 vs 7 prop measures, echo = F, warning = F, message = F}
...
# p.adjust(test$p.value, method = "holm")
...
Unlike the count-based measures, the 10 proportional measures from each recording-type are readily comparable without further computation. Descriptively, within recording type, the differences month-to-month were small. Indeed, as with the derived count measures, there were no significant differences between month six and seven within audio or video recordings (all adjusted-p>.05. The pattern of correlations differed somewhat from the derived counts: for the audio recordings, `r sum(p.adjust(pca_ps, method = "holm")<0.05)`/10 proportional measures correlated from month 6 to month 7 (for these seven measures: Kendall’s tau range: `r round(min(pca_taus),2)`-`r round(max(pca_taus),2)`, adjusted-p<.05; for object co-presence, questions, and short-phrases adjusted-p>.05). For video data, only proportion of input from mom and from dad correlated from month 6 to month 7 (Kendall’s tau = .45 and .63, respectively; adjusted-p<.05). Thus, overall, at the group level, the proportional metrics were very stable month-to-month within recording type, but the correlations between the proportional measures across children at month six and seven were variable, especially for video-recordings.
...
```
```{r propvals_countvals, echo = F, warning = F, message = F}
...
top100_avcortest <- cor.test(top100av_spread$audio, top100av_spread$video, method = "spearman")# sig cor, but ~1/2 are 0s
# top100av_spread %>% 
...
```{r word tallies, echo = F, message = F, warning = F}
...
morethanonce<- sixseven_basiclevel_home_data %>% 
  group_by(object) %>% 
  tally() %>% 
  filter(n>1)
...
```{r tallying graphs, echo = F, warning = F, mesasge = F}
...
```{r wilcox tests AV prop, echo = F, warning = F, message = F}
...
```{r prop AV cor tests, echo = F, warning = F, message = F}
