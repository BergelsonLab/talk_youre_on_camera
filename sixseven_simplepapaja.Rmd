...
We next normed our count values by the number of minutes in each. For example, if an infant heard 500 noun-tokens in 800 minutes of non-silent audio-recording, and 200 in 60 minutes of videos, this was normed to .62 and 3.3 noun-tokens per minute, respectively. Unlike for the video-fractions, this allows us to retain zero values, rendering more readily interpretable results across our count and proportional measures.
...
With the normed data, `r w_sigps`/12 of our metrics occured at significantly lower rates in audio recordings than video recordings (adjusted-p<.05). The remaining metric, number of nouns from fathers, was statistically indistinguishable across recording types (adjusted-p>.05). Thus, overall, per unit time, infants heard less noun input across our metrics of quantity, talker, utterance-type and object presence in audio recordings than in videos (per unit time).

Looking next at correlations, we find that `r c_sigps`/12 metrics correlated in audio vs. video data; number of nouns per minute heard in singing did not. The size of the correlations was significant, but relatively moderate (excluding singing, *M* = `r tau_mean`, `r tau_min` - `r tau_max`, all adjusted-p<.05). See Table \@ref(tab:normtable) and Figures \@ref(fig:gr-derived-counts-diff) and \@ref(fig:gr-derived-counts-corr). 
