...
#the table below should be identical to vboost_mean but isn't, probably because of NAs?
# much simpler code though--need to consolidate with code for vboost
# countvals_table <- countvals_long_collapsed %>% 
#   group_by(audio_video, count_meas) %>% 
#   summarise(mnv = mean(countval)) %>% 
#   spread(audio_video, mnv) %>% 
#   mutate(count_inflation = video/audio) %>% 
#   dplyr::select(count_meas, count_inflation) %>% 
