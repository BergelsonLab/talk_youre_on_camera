...
#* fix figure/table rendering positions
...
cap <- "Raw count of noun measures across audio-recordings (solid borders) and videos (dashed borders). Top panel labels reflects measure type (op = object presence; utt = utterance-type, quant = quantity, speaker_num = number of speakers). Bars appear in legend order."
```
...
cap <- "Log-scaled counts of the top 100 words in audio- and video-recordings. Each node represents the averaged count, across all participants in both months, of each noun. .1 was added before taking logs to include 0 counts. Words in blue occurred 0 times in one recording type; words in pink were attested in both recording types. Nodes are jittered for visual clarity, with grey lines indicating node location on axes."
```  
...
```{r top10noun-freq, fig.cap = cap, echo = F, warning = F, fig.height = 3, fig.width = 7}
top10_graph_collapsed
cap<-"Top 10 words by recording type. Each node represents the frequency count of a top audio or video word over both months (x-axis) and count of families where that word was said (out of 44) across months (y-axis)."
#overall_top10
```  
