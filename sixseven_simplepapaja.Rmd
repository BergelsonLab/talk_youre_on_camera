...
>>>>>>> 2d8a82ad5f70b24d2ed2af33c3e0b32a8aadc24c
getmode <- function(v) {
   uniqv <- unique(v)
...
<<<<<<< HEAD
```{r measures-tab, comment = F, message = F, warning = F, echo = F}
measures_table2
=======
```{r measures-table, echo = F, warning = F, message = F, fig.cap="Derived count and proportion measures"}
measures_table
>>>>>>> 2d8a82ad5f70b24d2ed2af33c3e0b32a8aadc24c
```
...
We next normed our count values by the number of minutes in each. For example, if an infant heard 500 noun-tokens in 800 minutes of non-silent audio-recording, and 200 in 60 minutes of videos, this was normed to .62 and 3.3 noun-tokens per minute, respectively. Zero values are retained within the normed count data.\footnotes{We made one exception for an infant with no father. This infant's data was excluded from 'father' values in \@ref(tab:propna-missing-tables) and elsewhere}
...
w_sigps <- count(ws %>% filter(pval_adj<.05))
c_sigps <- count(cs %>% filter(pval_adj<.05))
