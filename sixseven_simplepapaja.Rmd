...
#to do's: 
# caption tables (fix if too wide for page) and position in reasonable places for flow of text 
...
apa_table(vars_with_NAs, caption = "Proportion of zero values", small = T, note = "V indicates videos, A indicates audio-recordings")
```
...
apa_table(countvals_normed_table,cap="Count values, normed by number of minutes in recording", small = T)
```
...
```{r zipfian, fig.cap = "Zipfian word frequency distributions", echo = F, warning = F, message= F, fig.height = 2, fig.width = 3}
zipf_av
```
...
```{r top10noun-freq, fig.cap = "Top 10 words by month and recording type", echo = F, warning = F, fig.height = 3, fig.width = 7}
top10_graph_collapsed
#may need to make an official table of the top 10 av/67 words, ask Shannon/Andrei
#overall_top10
```  
