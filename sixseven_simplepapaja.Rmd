...
```{r recording-ages-table, comment=F, message=F, hide = T, warning = F, echo = F, fig.cap="Infant ages at recordings and lab visits"}
...
```{r measures-tab, echo = F, warning = F, message = F, fig.cap="Derived count measures"}
measures_table2
```
...
Before assessing how our 12 measures of noun input scaled between hour-long video-recordings and daylong audio-recordings, we analyzed recording lengths. Modally, videos were an hour (`r vidmode` min, *M*=`r vidM_SD_R`), and audio-recordings were 16 hours (`r audmode` min, *M*=`r audM_SD_Rmin`), the maximum capacity of the LENA device. While audio-recordings began when children awoke, we further estimated the onsets and offsets of daytime naps by removing the 'silent' portions of the recordings (see Methods). This provides an estimated upper-limit on infants' awake (i.e. non-silent) time (Mode = `r audmode_nosil_hr*60` min., *M* = `r audnosilM_SD_R` ). This comports with established norms for 6–8-month-olds in the US (**ADD REF:** Mandel et al, 2010), which are 180 minutes of daytime sleep, and 600 minutes of nighttime sleep. Infants were always awake during video recordings (save one infant, who fell asleep before the recording-hour ended; that video was stopped at sleep onset).
...
```{r vboost-table, echo = F, warning = F, message = F, fig.cap = "Video Boost",}

vboost_table
# TO DO: Singing and reading show up as "Inf"?
```
...
We next normed our count values by the number of minutes in each. For example, if an infant heard 500 noun-tokens in 800 minutes of non-silent audio-recording, and 200 in 60 minutes of videos, this was normed to .62 and 3.3 noun-tokens per minute, respectively. Zero values are retained within the normed count data.\footnote{We made one exception for an infant with no father. This infant's data was excluded from 'father' values in \@ref(tab:propna-missing-tables) and elsewhere}
...
countvals_normed_table$norm_meas <- c("Object presence","Mother","Father","Declaratives","Questions","Short phrases","Singing","Reading","Imperatives","Type count","Token count","Speaker count")

countvals_normed_table <- countvals_normed_table %>%
...
Looking next at correlations, we find that `r c_sigps`/12 metrics correlated in audio vs. video data; nouns per minute heard from fathers and in singing did not. The size of the correlations was significant, but relatively moderate (excluding the two non-significant metrics, *M* = `r tau_mean`, `r tau_min` - `r tau_max`, all adjusted-p<.05). See Table \@ref(tab:normtable) and Figures \@ref(fig:gr-derived-counts-diff) and \@ref(fig:gr-derived-counts-corr). 
